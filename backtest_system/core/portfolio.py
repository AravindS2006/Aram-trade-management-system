...
class Portfolio:
    ...
    def update_position(self, symbol: str, qty: int, price: float, timestamp: datetime, 
                       transaction_cost: float = 0.0, tag: str = ""):
        price = round(price, 3)
    ...
    def generate_tearsheet(self):
        ...
        total_charges = trades_df["Taxes & Charges"].sum() if "Taxes & Charges" in trades_df.columns else 0.0
    ...
    def _calculate_sharpe(self, trades_df):
        if len(trades_df) < 2:
            return 0.0
        trades_df["Return %"] = trades_df["Net PnL"] / (trades_df["Entry Price"] * trades_df["Quantity"].abs())
