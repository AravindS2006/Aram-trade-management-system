...
DEFAULT_SECURITY_MASTER = "data/raw/dhan/security_master.csv"
...
class DhanSecurityMaster:
    ...
    def __init__(self, master_file: str | None = None):
        master_file = master_file or os.getenv("DHAN_SECURITY_MASTER_CSV", DEFAULT_SECURITY_MASTER)
    ...
    def get_security_id(self, symbol: str, exchange_segment: str = "NSE_EQ") -> str | None:
...
class DhanSandboxClient:
    ...
    def place_market_order(
        ...
        try:
            ...
            resp = self.dhan.place_order(
                security_id=sec_id,
                exchange_segment=getattr(self.dhan, exchange_segment, self.dhan.NSE),
                transaction_type=dhan_side,
                quantity=quantity,
                order_type=self.dhan.MARKET,
                product_type=getattr(self.dhan, product_type, self.dhan.CNC),
                price=0,
            )
    ...
    def close_position(self, symbol: str, exchange_segment: str = "NSE_EQ") -> Order | None:
        positions = self.get_positions()
        if positions.empty:
            return None
        if "tradingSymbol" not in positions.columns:
            return None
    ...
    def get_ltp(self, symbol: str, exchange_segment: str = "NSE_EQ") -> float | None:
        sec_id = self.master.get_security_id(symbol, exchange_segment)
        if not sec_id:
            return None
        try:
            r = self.dhan.ohlc_data(
                security_id=sec_id, exchange_segment=exchange_segment, instrument_type="EQUITY"
            )
            if r and r.get("status") == "success":
                return float(r.get("data", {}).get("last_price", 0))
        except Exception as exc:
            logger.debug(f"LTP fetch failed for {symbol}: {exc}")
        return None
