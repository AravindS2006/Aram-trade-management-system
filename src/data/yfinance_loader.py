...
NSE_HOLIDAYS = {
    "2024-01-26","2024-03-25","2024-03-29","2024-04-14","2024-04-17","2024-04-21",
    "2024-05-23","2024-06-17","2024-07-17","2024-08-15","2024-10-02","2024-10-14",
    "2024-11-01","2024-11-15","2024-12-25",
    "2025-01-26","2025-02-26","2025-03-14","2025-03-31","2025-04-10","2025-04-14",
    "2025-04-18","2025-05-01","2025-08-15","2025-10-02","2025-10-21","2025-11-05","2025-12-25",
}
...
class YFinanceLoader:
    ...
    def fetch_universe(self, symbols: List[str], start: str, end: Optional[str] = None,
        ...
        for i in range(0, len(to_download), 50):
            ...
            try:
                ...
                raw = yf.download(tickers, start=start, end=end, interval=interval,
                                  auto_adjust=self.auto_adjust, group_by="ticker",
    ...
    def fetch_index(self, index_name: str = "NIFTY50", start: str = "2020-01-01",
                    end: Optional[str] = None, interval: str = "1d") -> pd.DataFrame:
        INDEX_MAP = {"NIFTY50":"^NSEI","NIFTYBANK":"^NSEBANK","SENSEX":"^BSESN","NIFTYIT":"^CNXIT"}
    ...
    def get_nifty50_symbols(self) -> List[str]:
        return ["RELIANCE.NS","TCS.NS","HDFCBANK.NS","INFY.NS","HINDUNILVR.NS",
                ...
                "BAJFINANCE.NS","POWERGRID.NS","NTPC.NS","TATAMOTORS.NS","M&M.NS",
                "TECHM.NS","ADANIPORTS.NS","ONGC.NS","COALINDIA.NS","JSWSTEEL.NS",
                "TATASTEEL.NS","HINDALCO.NS","BPCL.NS","GRASIM.NS","DIVISLAB.NS",
                "BAJAJFINSV.NS","CIPLA.NS","DRREDDY.NS","EICHERMOT.NS","HEROMOTOCO.NS",
                "APOLLOHOSP.NS","ADANIENT.NS","TATACONSUM.NS","BRITANNIA.NS",
                "SBILIFE.NS","HDFCLIFE.NS","UPL.NS","INDUSINDBK.NS","BAJAJ-AUTO.NS","LTI.NS"]
    ...
    def _load_cache(self, symbol: str, interval: str, start: str, end: str) -> Optional[pd.DataFrame]:
        p = self._cache_path(symbol, interval)
        if not p.exists(): return None
        try:
            cached = pd.read_parquet(p)
            if cached.empty: return None
            if (cached.index[0].date() <= pd.Timestamp(start).date() and
                    cached.index[-1].date() >= pd.Timestamp(end).date() - timedelta(days=5)):
