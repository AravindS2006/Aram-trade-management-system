...
class IntradayBacktester:
    ...
    """
    def __init__(self, data_with_signals: pd.DataFrame, initial_capital: float = 100000, 
                 risk_per_trade_pct: float = 0.01, sl_pct: float = 0.01, tp_pct: float = 0.02, 
                 tsl_pct: float = 0.005, slippage: float = 0.0005, quantity: int = 1, commission: float = 20.0):
    ...
    def run(self) -> tuple[pd.DataFrame, dict]:
        ...
        for row in self.data.itertuples():
            ...
            if position != 0 and current_time >= self.square_off_time:
                ...
                # Apply slippage
                exit_price = exit_price * (1 - self.slippage) if position == 1 else exit_price * (1 + self.slippage)
                exit_price = round(exit_price, 3)

                self.portfolio.update_position(symbol, -position * current_qty, exit_price, dt, 
                                               transaction_cost=self.commission, tag="TIME_EXIT")
                position = 0
                current_qty = 0
                continue
                ...
                if position == 1 and low <= current_sl:
                    exit_price = round(current_sl * (1 - self.slippage), 3) # Execution at SL level with slippage
                    exit_triggered = True
                    tag = "SL_HIT"
                ...
                if exit_triggered:
                    self.portfolio.update_position(symbol, -position * current_qty, exit_price, dt, 
                                                   transaction_cost=self.commission, tag=tag)
                    position = 0
                    current_qty = 0
                    continue
            ...
            if position == 0 and current_time < self.square_off_time:
                if exec_signal == 1: # Buy
                    ...
                    capital_at_risk = self.portfolio.total_value({symbol: c}) * self.risk_per_trade_pct
                    sl_amount = entry_price * self.sl_pct
                    current_qty = max(1, int(capital_at_risk / sl_amount)) if sl_amount > 0 else self.fixed_quantity
                    
                    position = 1
                    self.portfolio.update_position(symbol, current_qty, entry_price, dt, 
                                                   transaction_cost=self.commission, tag="LONG_ENTRY")
                    ...
                    position = -1
                    self.portfolio.update_position(symbol, -current_qty, entry_price, dt, 
                                                   transaction_cost=self.commission, tag="SHORT_ENTRY")
                    
                    # Set Risk Levels
                    current_sl = entry_price * (1 + self.sl_pct)
                    current_tp = entry_price * (1 - self.tp_pct)
                    max_favorable_price = entry_price # For short, lower is better, but we track low
