...
class BTStrategyWrapper(bt.Strategy):
    ...
    params = dict(aram_strategy=None, precomputed_signals=None, print_log=False)

    def __init__(self):
        self.aram = self.params.aram_strategy
        self.orders = {}
        self.trade_log = []
        self._precomputed: dict[str, pd.Series] = self.params.precomputed_signals or {}
    ...
    def next(self):
        for data in self.datas:
            ...
            bar_idx = len(data) - 1
            sig = int(
                self._precomputed.get(sym, pd.Series()).iloc[bar_idx]
                if sym in self._precomputed and bar_idx < len(self._precomputed[sym])
                else 0
            )
    ...
    def notify_order(self, order):
        ...
        if (
            order.status in [order.Completed]
            and sym in self.orders
            or order.status in [order.Canceled, order.Rejected]
            and sym in self.orders
        ):
    ...
    def notify_trade(self, trade):
        if trade.isclosed:
            self.trade_log.append(
                {
                    "strategy": getattr(self.aram, "NAME", "unknown"),
                    ...
                    "size": trade.size,
                    "pnl": trade.pnl,
                    "return_pct": (trade.pnl / (abs(trade.size) * trade.price))
                    if trade.size
                    else 0,
...
class BacktraderRunner:
    ...
    def run(
        ...
        cerebro.addstrategy(
            BTStrategyWrapper,
            aram_strategy=strategy,
            precomputed_signals=sig_map,
        )
        ...
        return self._extract(
            results_list[0], strategy, start_val, end_val, data, benchmark_data, sig_map
        )
    ...
    def _extract(self, strat, strategy, start_val, end_val, data, benchmark_data, sig_map):
        ...
        try:
            ...
            n = win_rate = avg_win = avg_loss = pf = 0
        try:
            ...
            sortino = (
                float((tr_returns.mean() / downside.std()) * np.sqrt(252))
        ...
        r = BacktestResults(
            ...
            start_date=str(ref.index[0].date()),
            end_date=str(ref.index[-1].date()),
            ...
            engine="backtrader",
            bars_loaded=int(len(ref)),
            warmup_required=int(strategy.get_warmup_period()),
            diagnostics={
                "buy_signals": int(np.nansum([(s == 1).sum() for s in sig_map.values()])),
                "sell_signals": int(np.nansum([(s <= 0).sum() for s in sig_map.values()])),
            },
        )
        r.print_summary()
        return r
